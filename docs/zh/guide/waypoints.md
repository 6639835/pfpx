...
| `FKB`        | IATA 代码            |                                                         |
| `0`          | 类型代码             | 0 = 机场                                                |
| `EDSB`       | ICAO 代码            | 与上方 ICAO 相同                                        |
| `ED`         | 区域代码             |                                                         |
| `P`          | 部分代码             | 机场（参考 ARINC424-18 5.4）                            |
| `1`          | 标志                 | 0=航路，1=终端，机场始终为1                                |
| `C`          | 机场类别             | C = 民用，M = 军用，P = 私人                            |
| `ED`         | 区域代码             |                                                         |
| `KARLSRUHE/BADEN-BADEN`    | 名称                 |                                                         |
| `003214`     | 数据库 ID            | 唯一标识                                                |
| `+48779353`  | 纬度                 | WGS-84，小数度 × 1e6，+ = 北纬，- = 南纬 → 40.073333°N  |
| `+008080500` | 经度                 | WGS-84，小数度 × 1e6，+ = 东经，- = 西经 → 116.598333°E |
| `+00409`     | 海拔高度（英尺）     | + = 海平面以上，- = 海平面以下                          |
| `09843`      | 最长跑道长度（英尺） | 例如 12,467 ft                                          |
| `EDGG`       | 低空FIR   |                                                         |
| `EDUU`       | 高空FIR   |                                                         |
| `9850`       | 过渡高度（ft）       |                                                         |
